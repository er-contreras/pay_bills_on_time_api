...
RSpec.describe EmailJob, type: :job do
  describe '#perform' do
    it 'sends monthly bill notifications for bills with notification on the current date' do
      ...
      user = create(:user)
      bill = create(:bill, user:, date: Date.today)
      create(:bill_notification, bill:, notification: true)

      # Ensure that the mailer is expected to receive :deliver_later
      allow(MonthlyBillNotificationMailer).to receive(:monthly_bill_notification).and_return(double(
                                                                                               'Mailer',
                                                                                               deliver_later: true
                                                                                             ))
      # Perform the job
      ...
      expect(MonthlyBillNotificationMailer).to have_received(:monthly_bill_notification).once.with(user.email,
                                                                                                   Date.today)
    end
    ...
    it 'does not send notifications for bills with notification turned off' do
      ...
      user = create(:user)
      bill = create(:bill, user:, date: Date.today)
      create(:bill_notification, bill:, notification: false)
